...
import { createInterview, getInterviewById } from "../services/interviews.service";
...
router.get("/token", async (req: Request, res: Response): Promise<void> => {
  try {
    const room = (req.query.room as string) || "devcareer-livekit-room";
    const identity = (req.query.identity as string) || `user_${Math.random().toString(36).substring(2, 7)}`;
...
router.get("/interview-details", async (req: Request, res: Response): Promise<void> => {
